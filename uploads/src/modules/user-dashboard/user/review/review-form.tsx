      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "translator" | "company")}>
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="translator" className="gap-2">
            ...
            <span className="hidden sm:inline">{t("tabs.translatorDesktop")}</span>
            <span className="sm:hidden">{t("tabs.translatorMobile")}</span>
        ...
        <TabsContent value="company" className="mt-4">
          <CompanyReview onSubmit={onCompanyReviewSubmit} onCancel={onCancel} isSubmitted={company} />
        </TabsContent>
      </Tabs>
    </div>
  )
